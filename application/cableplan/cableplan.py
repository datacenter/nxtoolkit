...
class CABLEPLAN:
    ...
    def get(cls, source):
        ...
        a Session, it will read the corresponding Nexus to get the cable plan.
        ...
        elif isinstance(source, NX.Session):
            system = NX.System.get(source)
            return cls._parse_nexus(source, str(system.name))
        else:
            raise TypeError('source must of type str or type NX.Session.  Instead was '+type(source))
    ...
    @classmethod
    def _parse_nexus(cls, session, host_name):
        neighbors = NX.LinkNeighbors.get(session)              
        # pod.populate_children(deep=True)
        cable_plan = cls()
        cable_plan._build_nexus(neighbors, host_name)
        return cable_plan
    ...
    def _build_nexus(self, neighbors, host_name):
        ...
        :returns: None
        """
        # TODO (tnahak) switch type is hard coded, Need to get from device
        source_switch = CpSwitch(host_name, 'n9k', host=True)
        self.add_switch(source_switch)
        for n in neighbors:
            new_switch = CpSwitch(n.attributes['sysName'],
                                     n.attributes['platId'])
            
            self.add_switch(new_switch)
        
            source_chassis = source_switch
            dest_chassis = new_switch
            source_port = re.sub(r'eth|Ethernet', 'Eth', n.attributes['id'])
            dest_port = re.sub(r'eth|Ethernet', 'Eth', n.attributes['portId'])
    ...
    def _parse_xml_chassis_info(self, node):
        ...
        switch = CpSwitch(chassis_name, chassis_type, host=True)
...
class CpSwitch(object):
    ...
    def __init__(self, name, chassis_type=None, host=False, parent=None):
        self.host = host
        self.name = name
...
def compare_cable_plans(session, file1, file2=None):
    if file2:
        ...
        resp = session.login()
        if not resp.ok:
            print '%% Could not login to Nexus'
            sys.exit(1)
...
def main():
    description = 'Simple application that logs on to the Nexus and displays stats for all of the Interfaces.'
