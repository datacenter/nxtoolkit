...
from a switch, export previously imported cable plans to a
...
When you want to create a cable plan from the current running Switch 
simply do the following::
    
  cp = CABLEPLAN.get(session)

Where ``session`` is a Switch session object generated using the
