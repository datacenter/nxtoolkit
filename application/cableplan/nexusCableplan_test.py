...
class Test_ParseXML(unittest.TestCase):
    ...
    def get_expected_xml(self):
        expected_xml = ['<?xml version="1.0" encoding="UTF-8"?>',
                        ...
                        '        <CHASSIS_INFO sourceChassis="leaf4-dc1" type="n9k">',
                        '            <LINK_INFO    sourcePort="Eth1/33"     destChassis="leaf3-dc1"     destPort="Eth1/33"/>',
                        '            <LINK_INFO    sourcePort="Eth2/1"     destChassis="Spine1-DC1"     destPort="Eth1/1"/>',
                        '            <LINK_INFO    sourcePort="Eth1/46"     destChassis="CC30-9372-28"     destPort="Eth1/3"/>',
                        '        </CHASSIS_INFO>',
                        '    </DATA_CENTER>',
                        '</CISCO_NETWORK_TYPES>']
        return expected_xml
    ...
    def test_datacenter(self):
        fname = self.initXML()
        cp = cableplan.CABLEPLAN.get(fname)
        self.assertEqual(cp.networkLocation, 'None')
    ...
    def test_chassisinfo(self):
        fname = self.initXML()
        cp = cableplan.CABLEPLAN.get(fname)
        switch = cp.get_switch('leaf4-dc1')
        self.assertEqual(switch.name, 'leaf4-dc1')
        self.assertEqual(switch.chassis_type, 'n9k')
        self.assertEqual(switch.host, True)
    ...
    def test_link_info(self):
        fname = self.initXML()
        cp = cableplan.CABLEPLAN.get(fname)
        switch = cp.get_switch('leaf3-dc1')
        self.assertEqual(switch.name, 'leaf3-dc1')
        self.assertEqual(switch.chassis_type, None)
        self.assertEqual(switch.host, False)

        switch = cp.get_switch('Spine1-DC1')
        self.assertEqual(switch.name, 'Spine1-DC1')
        self.assertEqual(switch.chassis_type, None)
        self.assertEqual(switch.host, False)

        switch = cp.get_switch('CC30-9372-28')
        self.assertEqual(switch.name, 'CC30-9372-28')
        self.assertEqual(switch.chassis_type, None)
        self.assertEqual(switch.host, False)
        ...
        for switch in switches:
            self.assertIn(switch.name, ['leaf3-dc1', 'Spine1-DC1', 'CC30-9372-28', 'leaf4-dc1'])
            self.assertNotIn(switch.name, processed)
            processed.add(switch.name)

        links = cp.get_links()
        self.assertEqual(len(links), 3)
        valid_links = ['(leaf3-dc1-Eth1/33,leaf4-dc1-Eth1/33)',
                       '(Spine1-DC1-Eth1/1,leaf4-dc1-Eth2/1)',
                       '(CC30-9372-28-Eth1/3,leaf4-dc1-Eth1/46)']
    ...
    def test_link_create(self):
        switch1 = cableplan.CpSwitch('leaf3-dc1')
        switch2 = cableplan.CpSwitch('new')
    ...
    def test_link_create_error(self):
        switch1 = cableplan.CpSwitch('leaf3-dc1')
        switch2 = cableplan.CpSwitch('Spine1-DC1')
    ...
    def test_switch_create(self):
        switch1 = cableplan.CpSwitch('leaf3-dc1')
        switch2 = cableplan.CpSwitch('Leaf2')
        switch3 = cableplan.CpSwitch('leaf3-dc1')
        self.assertEqual(switch1.get_name(), 'leaf3-dc1')
        self.assertFalse(switch1.host)
    ...
    def test_switch_add(self):
        ...
        self.assertTrue(test_switch1.host)
    ...
    def test_add_switch_error(self):
        ...
        switch1 = cableplan.CpSwitch('leaf3-dc1', parent=cp1)
    ...
    def test_link_add(self):
        ...
        spine1 = cp.get_switch('leaf4-dc1')
        leaf1 = cp.get_switch('leaf3-dc1')
        # leaf2 = cp.get_switch('Leaf2')

        base_links = cp.get_links()
        self.assertEqual(len(base_links), 3)
        ...
        self.assertEqual(len(base_links), 5)
...
class Test_portset(unittest.TestCase):
    def test_port_equal_case_insensitive(self):
        spine1 = cableplan.CpSwitch('leaf4-dc1')
        leaf1 = cableplan.CpSwitch('leaf3-dc1')
    ...
    def test_hasPortInCommon(self):
        spine1 = cableplan.CpSwitch('leaf4-dc1')
        leaf1 = cableplan.CpSwitch('leaf3-dc1')
        leaf2 = cableplan.CpSwitch('Spine1-DC1')
        leafz = cableplan.CpSwitch('Zleaf3-dc1')
    ...
    def test_overlapping(self):
        spine1 = cableplan.CpSwitch('leaf4-dc1', host=True)
        leaf1 = cableplan.CpSwitch('leaf3-dc1')
        leaf2 = cableplan.CpSwitch('Spine1-DC1')
        leaf3 = cableplan.CpSwitch('CC30-9372-28')
...
class Test_switch(unittest.TestCase):
    def test_name_change(self):
        ...
        link1 = cableplan.CpLink(source_chassis=spine1, source_port='Eth1/3', dest_chassis=leaf1, dest_port='Eth1/1')
        # link2 = cableplan.CpLink(source_chassis=spine1, source_port='eTh1/3', dest_chassis=leaf1, dest_port='eth1/1')
        self.assertEqual('(leaf3-dc1-Eth1/1,leaf4-dc1-Eth1/3)', link1.get_name())
        self.assertEqual('leaf4-dc1', spine1.get_name())

        spine1.set_name(new_name)
        self.assertEqual('(leaf3-dc1-Eth1/1,' + new_name + '-Eth1/3)', link1.get_name())
...
class Test_difference_switch(unittest.TestCase):
    def test_difference(self):
        ...
        spine1a = cableplan.CpSwitch('leaf4-dc1', host=True)
        spine1b = cableplan.CpSwitch('leaf4-dc1', host=True)
        leaf1 = cableplan.CpSwitch('leaf3-dc1')
        leaf2 = cableplan.CpSwitch('Spine1-DC1')
        leaf3 = cableplan.CpSwitch('CC30-9372-28')
...
class Test_export(unittest.TestCase):
    ...
    def test_export(self):
        ...
        leaf2 = cableplan.CpSwitch('Spine1-DC1')
        leaf3 = cableplan.CpSwitch('CC30-9372-28')
        spine1 = cableplan.CpSwitch('leaf4-dc1', chassis_type='n9k', host=True)
        cp.add_switch(leaf1)
        cp.add_switch(leaf2)
        cp.add_switch(leaf3)
        cp.add_switch(spine1)
        ...
        cp.add_link(
            cableplan.CpLink(source_chassis=spine1, source_port='Eth1/33', dest_chassis=leaf1, dest_port='Eth1/33'))
        cp.add_link(cableplan.CpLink(source_chassis=spine1, source_port='Eth2/1', dest_chassis=leaf2,
                                     dest_port='Eth1/1'))
        cp.add_link(cableplan.CpLink(source_chassis=spine1, source_port='Eth1/46', dest_chassis=leaf3,
                                     dest_port='Eth1/3'))
    ...
    def test_import(self):
        ...
        Leaf2 = cableplan.CpSwitch('Spine1-DC1')
        Leaf3 = cableplan.CpSwitch('CC30-9372-28')
        Spine1 = cableplan.CpSwitch('Aleaf4-dc1', chassis_type='n9k', host=True)
...
@unittest.skipIf(LIVE_TEST is False, 'Not performing live Nexus testing')
class TestLiveNexus(unittest.TestCase):
    def login_to_nexus(self):
        """Login to the Nexus
...
class Test_compare_cp(unittest.TestCase):
    def test_basic(self):
        ...
        spine1a = cableplan.CpSwitch('leaf4-dc1', host=True)
        spine1b = cableplan.CpSwitch('leaf4-dc1', host=True)
        leaf1a = cableplan.CpSwitch('leaf3-dc1')
        leaf2a = cableplan.CpSwitch('Spine1-DC1')
        leaf3a = cableplan.CpSwitch('CC30-9372-28')
        leaf1b = cableplan.CpSwitch('leaf3-dc1')
        leaf2b = cableplan.CpSwitch('Spine1-DC1')
        leaf3b = cableplan.CpSwitch('CC30-9372-28')

        cp1.add_switch(spine1a)
        self.assertEqual(len(cp1.difference_switch(cp2)), 1)
    ...
    def test_any(self):
        ...
        spine2a = cableplan.CpSwitch('leaf4-dc1', host=True)
        spine2b = cableplan.CpSwitch('leaf4-dc1', host=True)
        leaf1a = cableplan.CpSwitch('leaf3-dc1')
        leaf1b = cableplan.CpSwitch('leaf3-dc1')
