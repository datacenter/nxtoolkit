...
################################################################################
"""
Used to get the NX and MySQL login credentials from the command
...
class Credentials(object):
    ...
    """
    def __init__(self, qualifier='switch', description=''):
        def set_default(key):
            """
            Check for the following:
             ...
             - credentials.py file
            """
            if 'NX_' + key.upper() in os.environ.keys():
                return os.environ['NX_' + key.upper()]
            else:
                try:
                    import credentials
                except ImportError:
        ...
        if 'switch' in qualifier:
            ...
            self._parser.add_argument('-p', '--password',
                                      ...
                                      help='NX login password.')
            self._parser.add_argument('--snapshotfiles', nargs='+',
                                      help='NX configuration files')
    ...
    def verify(self):
        ...
        """
        if 'switch' in self._qualifier and self._args.snapshotfiles is None:
            if self._args.login is None:
                self._args.login = self._get_from_user('NX login username: ')
            if self._args.url is None:
                self._args.url = self._get_from_user('NX URL: ')
            if self._args.password is None:
                self._args.password = self._get_password('NX Password: ')
